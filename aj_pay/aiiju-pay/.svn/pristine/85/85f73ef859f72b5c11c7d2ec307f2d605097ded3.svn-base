...
import com.aiiju.pay.business.zfb.builder.AlipayTradeCancelRequestBuilder;
...
public abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    public AlipayF2FRefundResult tradeRefund(AlipayTradeRefundRequestBuilder builder) {
        ...
        request.putOtherTextParam("app_auth_token", builder.getAppAuthToken());
        request.setBizContent(builder.toJsonString());
        log.info("退款时apptoken---->:" + builder.getAppAuthToken());
    ...
    public AlipayF2FPrecreateResult tradePrecreate(AlipayTradePrecreateRequestBuilder builder) {
        ...
        request.putOtherTextParam("app_auth_token", builder.getAppAuthToken());
        request.setBizContent(builder.toJsonString());
        log.info("appuserTaken="+builder.getAppAuthToken()+",trade.precreate bizContent:" + request.getBizContent());
    ...
    protected AlipayTradeQueryResponse loopQueryResult(AlipayTradeQueryRequestBuilder builder) {
        ...
        for (int i = 0; i < AlipayUtil.MAX_QUERY_RETRY; i++) {
            Utils.sleep(AlipayUtil.QUERY_DURATION);
    ...
    protected boolean stopQuery(AlipayTradeQueryResponse response) {
        if (ResultConstant.SUCCESS.equals(response.getCode())) {
    ...
    protected void asyncCancel(final AlipayTradeCancelRequestBuilder builder) {
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < AlipayUtil.MAX_CANCEL_RETRY; i++) {
                    Utils.sleep(AlipayUtil.CANCEL_DURATION);
    ...
    protected AlipayTradePayResponse toPayResponse(AlipayTradeQueryResponse response) {
        ...
        payResponse.setCode(querySuccess(response) ? ResultConstant.SUCCESS : ResultConstant.FAILED);
        // 补充交易状态信息
    ...
    protected boolean tradeError(AlipayResponse response) {
        return response == null ||
                ResultConstant.ERROR.equals(response.getCode());
    }
}
