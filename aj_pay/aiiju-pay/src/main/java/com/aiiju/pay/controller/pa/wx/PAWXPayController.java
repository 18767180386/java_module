package com.aiiju.pay.controller.pa.wx;
...
@Controller
@RequestMapping("/pawx")
public class PAWXPayController {

    private static Logger logger = Logger.getLogger(PAWXPayController.class);
    ...
    public Result barCodePay(HttpServletRequest req) {
		Result result = null;
		/**
		 * 0, 查询openid,openkey,若无，则告知无签约银行
		 * 1，创建订单
		 * 2，创建成功后 接口提交交易信息
		 * 3，返回客户端成功或失败信息
		 * 
		...
		String payType = "52"; // 收款方式：平安微信条码
		Map createMap = this.createF2FOrder(req, payType, outTradeNo, tradeType);
		...
		order.setPmtTag("WeixinHZPA");
		order.setPmtName("微信支付(杭州平安)");
...
//		 保存交易时所用的授权账户
		...
		if (payOrderResultJson.getStatus() == 500) {
			 //更新订单失败
			logger.error("平安微信支付失败!!!");
			result = Result.build(40004, "平安微信支付失败!");
			...
			if("1".equals(payStatus)){
				...
				result = Result.build(10000, "平安微信支付成功!", null, null, deal.getSerialNum());
				...
				if("0".equals(payStatus)){
					   result = Result.build(40004, "微信支付失败,订单已取消");
				}else{
...
//			 保存交易时所用的授权账户
			...
			if(payOrderErrcode!=0){
				return	Result.build(40004,payOrderMsg);
			...
			if (payOrderResultJson.getStatus() == 500) {
				...
				result = Result.build(200,"银行微信转二维码成功", trade_qrcode, null, deal.getSerialNum());
	...
	public Result notifyResult(HttpServletRequest request, HttpServletResponse response) {
				...
				if(deal==null){
					logger.info("平安微信支付后回调（本地无此交易信息，所以不做任何操作）");
				}
...
private Map createF2FOrder(HttpServletRequest request, String receType, String outTradeNo,String tradeType) {
    ...
    public Result refund(HttpServletRequest req) {
		...
		String refundType = "5"; // 退款方式：平安微信退款
        Map createMap = this.createRefundOrder(req, refundType, originalOutTradeNo, newOutTradeNo,tradeType,refundOutNo);
		...
		order.setRefundOrdName("平安银行微信退款");
      ...
      //  datamap.put("trade_no", order.getTradeNo());  
		...
		if (payOrderResultJson.getStatus() == 500) {
			 //更新订单失败
			logger.error("平安微信退款失败!!!");
			result = Result.build(40004, "平安微信退款失败!");
			return payOrderResultJson; // 返回 提审失败的具体信息
		} else {
			...
			if("1".equals(status)){
				  this.dealService.updateRefundDeal(newOutTradeNo, ord_no);
			        ...
			        result = Result.build(10000, "平安微信退款成功!", true);
			}else{
				result = Result.build(40004, "平安微信退款失败!");
			}
    ...
    private Map createRefundOrder(HttpServletRequest req, String refundType, String originalOutTradeNo,
            String newOutTradeNo,String tradeType,String outRefundNo) {
        // 创建条码支付流水订单

        Deal deal = PADealBuilder.buildRefundOrder(req, refundType, originalOutTradeNo, newOutTradeNo,tradeType,outRefundNo);
