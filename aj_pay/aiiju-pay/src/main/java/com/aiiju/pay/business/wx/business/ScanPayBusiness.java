...
import com.aiiju.pay.business.wx.common.report.ReporterFactory;
...
import com.aiiju.pay.business.wx.protocol.pay_protocol.ScanPayResData;
...
/**
 * 
 * @ClassName: ScanPayBusiness 
 * @Description: 刷卡支付
 * @author 小飞 
 * @date 2016年12月7日 下午4:54:06 
 *
 */
public class ScanPayBusiness {
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        if(payServiceResponseString == null) {
        	if(doOnePayQuery(subMchId,"",outTradeNo)) {
     ...
     * @throws Exception
     */
    private boolean doOnePayQuery(String subMchId,String transactionID,String outTradeNo) throws Exception {

        sleep(waitingTimeBeforePayQueryServiceInvoked);//等待一定时间再进行查询，避免状态还没来得及被更新

        String payQueryServiceResponseString;

        ScanPayQueryReqData scanPayQueryReqData = new ScanPayQueryReqData(subMchId,transactionID,outTradeNo);
    ...
    private boolean doOneReverse(String subMchId,String transactionID,String outTradeNo) throws Exception {
        ...
        ReverseReqData reverseReqData = new ReverseReqData(subMchId,transactionID,outTradeNo);
