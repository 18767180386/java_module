...
/* 	        border-bottom: 2px solid #42B5EE; */
  ...
  <body>
    <div class="container">
        <div class="header">
        	<img src="http://16794ui705.iok.la:32542/image/join.png"/>
        </div>
        <div class="nav">
        
           <div class="nav-title active" id="new-title"><%=fromPhone%>邀请您加入<%=shopName%>店铺成为店长</div>
            ...
            $("#newJoinBtn").on("click",function() {
				...
				$.post(host+"/note/check",{"phone":newPhone,"code":code},function(resp) {
					...
					$.post(host+"/user/inviteUser",{"phone":newPhone,"password":password,"storeId":"${storeId}"},function(res) {
						if (res.status == "200") {
						alert(222);
						var shopname_now = '<%=shopName%>';
						alert(shopname_now);
						var nowurl = host+"/user/inviteResult?shopName="+shopname_now;
						alert(nowurl);
