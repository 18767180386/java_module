...
public class DateUtil
{
    ...
    public static Date getDate1(String times) {
        long time = Long.parseLong(times);
    ...
    public static String getNowTime() {
        return StringUtil.isNull(System.currentTimeMillis() / 1000);
    }
     ...
     * 计算结束时间
     */
    public static Date getEndTime(Date starttime, int year,int month,int day) {
        Calendar end = Calendar.getInstance();
        end.setTime(starttime);
        if (end.get(Calendar.MONTH) + 1 > month) {
            end.add(Calendar.YEAR, year);
        } else if (end.get(Calendar.MONTH) + 1 == month) {
            if (end.get(Calendar.DATE) - day >= 0) {
                end.add(Calendar.YEAR, year);
            } else {
        ...
        end.set(Calendar.DATE, day - 1 );

        return end.getTime();
    }

    public static void main(String[] args) {
        System.out.println(getEndTime(new Date(),4,7,25));
    }

    public static int useYears(Date orderTime,int month,int day) {
        int year = 0;
        ...
        if (now.get(Calendar.MONTH) + 1 > month) {
            year = now.get(Calendar.YEAR) - orderT.get(Calendar.YEAR) + 1;
        } else if (now.get(Calendar.MONTH) + 1 == month) {
            if (now.get(Calendar.DATE) > day) {
                year = now.get(Calendar.YEAR) - orderT.get(Calendar.YEAR) + 1;
            } else {
                year = now.get(Calendar.YEAR) - orderT.get(Calendar.YEAR);
            }
        } else {
            year = now.get(Calendar.YEAR) - orderT.get(Calendar.YEAR);
        }
    ...
    public static String setMonDay(Date date,int month,int day){
    ...
    public static Date getEndTime(Date d,int year){
        Calendar cal = Calendar.getInstance();
        cal.setTime(d);
        cal.add(Calendar.YEAR, year);
    ...
    public static String getHourMinute1(){
        Calendar cal = Calendar.getInstance();
        int hour=cal.get(Calendar.HOUR_OF_DAY);//小时
        int minute=cal.get(Calendar.MINUTE);//分
        if(minute==60){
