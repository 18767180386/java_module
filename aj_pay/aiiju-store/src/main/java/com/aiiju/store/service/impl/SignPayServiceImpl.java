...
public class SignPayServiceImpl implements SignPayService {
	...
	public SignPay  getSignInfo(SignPay signPay){
	    ...
	    signPay.setParentStoreId(shop.getParentStoreId());
		...
		if("1".equals(shop.getShopType())){
			//直营店
			pamer =	signPayMapper.queryPAMerchant(shop.getParentStoreId());
		...
		if(pamer!=null){
			SignPay   pSignPay= new  SignPay();
			pSignPay.setStoreId(pamer.getStoreId());
			List<PAContract> list = signPayMapper.queryPAContractStatus(pSignPay);
