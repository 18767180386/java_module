...
//		}
    ...
    public Result updateInventory(String id,String inventory,String sign,String requtestTime) {
    	...
    	Result result = inventoryService.updateInventoryErp(map);
     ...
     * @return
     */
    @RequestMapping("/getErpMemberInfo")
    @ResponseBody
	public Result getErpMemberInfo(String keyword,String visitId) {

    	JSONObject json = null;
		// 判断keyword 是手机号 还是条形码
		if (keyword!=null) {
			//手机号码
				
				json = ErpMemBalanceUtil.getErpMemberInfo(keyword,visitId);
		}
        ...
        try {
        	...
        	}else if ("204".equals(code)) {//无数据
        		return Result.build(200, "操作成功");
			}else if ("405".equals(code)) {
        		return Result.build(500, "条形码失效");			
			}else{
				Object message =json.get("message");
        		 return Result.build(500, message.toString());
        	}
