...
<html>
  ...
  <body>
    <div class="container">
        ...
        <div class="nav">
           <div class="nav-title active" id="new-title">新用户</div>
      ...
      //测试
        //  var host = "http://16794ui705.iok.la:32542"; 
            $("#codeBtn").on("click",function() {
				...
				$.post(host+"/note/get",{"phone":newPhone,"type":"1"},function(resp) {
					if (resp.status == "201") {
						alert(resp.msg + ",请在[已有账号]下直接登录");
						clearInterval(si);
					} else if (resp.status == "401") {
						alert(resp.msg + ",请重新输入账号或密码");
            ...
            $("#newJoinBtn").on("click",function() {
				...
				$.post(host+"/note/check",{"phone":newPhone,"code":code},function(resp) {
					...
					$.post(host+"/user/inviteUser",{"userName":newPhone,"password":password,"storeId":"${storeId}"},function(res) {
						if (res.status == "200") {
             ...
             $("#oldJoinBtn").on("click",function() {
				...
				var that = this;
				$(that).attr("disabled","disabled").css("backgroundColor","#CDCDCD");
				$.post(host+"/user/inviteUser",{"userName":oldPhone,"password":password,"storeId":"${storeId}"},function(res) {
					$(that).removeAttr("disabled").css("backgroundColor","#01A0E8");
					if (res.status == "200") {
						window.location.href = host+"/user/inviteResult?shopName="+res.data;
